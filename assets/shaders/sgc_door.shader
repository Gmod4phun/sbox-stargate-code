...
PS
{
	...
	TextureAttribute( LightSim_DiffuseAlbedoTexture, g_tStripeMask )
	TextureAttribute( RepresentativeTexture, g_tStripeMask )
	float4 g_vDoorColor < UiType( Color ); UiGroup( ",0/,0/0" ); Default4( 1.00, 1.00, 1.00, 1.00 ); >;
	float4 g_vStripeColor < UiType( Color ); UiGroup( ",0/,0/0" ); Default4( 1.00, 1.00, 1.00, 1.00 ); >;
	bool g_bStripe < UiGroup( ",0/,0/0" ); Default( 0 ); >;
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float4 l_1 = g_vDoorColor;
		float4 l_2 = l_0 * l_1;
		float4 l_3 = g_vStripeColor;
		float4 l_4 = Tex2DS( g_tStripeMask, g_sSampler0, i.vTextureCoords.xy );
		float4 l_5 = lerp( l_2, l_3, l_4 );
		float4 l_6 = g_bStripe ? l_5 : l_2;
		float4 l_7 = Tex2DS( g_tNormal, g_sSampler0, i.vTextureCoords.xy );
		float3 l_8 = DecodeNormal( l_7.xyz );
		float4 l_9 = Tex2DS( g_tRoughness, g_sSampler0, i.vTextureCoords.xy );
		float4 l_10 = Tex2DS( g_tMetalness, g_sSampler0, i.vTextureCoords.xy );
		float4 l_11 = Tex2DS( g_tAmbientOcclusion, g_sSampler0, i.vTextureCoords.xy );
