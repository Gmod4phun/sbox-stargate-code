namespace Sandbox.Components.Stargate
{
	public class StargateMilkyWay : Stargate
	{
		...
		public override void OnStopDialingBegin()
		{
			...
			PlaySound(
				this,
				(ActiveChevrons > 0 || CurDialType is DialType.DHD)
					? GetSound("dial_fail")
					: GetSound("dial_fail_noclose")
			);

			if (Ring.IsValid() && Ring.IsMoving)
				Ring.SpinDown();
		}
		...
		public override async void BeginDialFast(string address)
		{
			...
			try
			{
				...
				var chevronsLoopDuration =
					(addrLen == 9) ? 4.40f : ((addrLen == 8) ? 4.25f : 3.90f);
				var chevronBeforeLastDelay =
					(addrLen == 9) ? 0.75f : ((addrLen == 8) ? 0.80f : 1.05f);
				var chevronAfterLastDelay =
					(addrLen == 9) ? 1.25f : ((addrLen == 8) ? 1.25f : 1.35f);
				...
				for (var i = 1; i < addrLen; i++)
				{
					...
					if (i == addrLen - 1)
						Ring.SpinDown(); // stop rotating ring when the last looped chevron locks
		...
		public override async void BeginInboundFast(int numChevs)
		{
			...
			try
			{
				...
				// duration of the dial until the gate starts opening - let's stick to 7 seconds for total (just like GMod stargates)
				// default values are for 7 chevron sequence
				var chevronsStartDelay =
					(numChevs == 9) ? 0.25f : ((numChevs == 8) ? 0.40f : 0.50f);
				var chevronsLoopDuration =
					(numChevs == 9) ? 6.75f : ((numChevs == 8) ? 6.60f : 6.75f);
				var chevronBeforeLastDelay =
					(numChevs == 9) ? 0.50f : ((numChevs == 8) ? 0.60f : 0.50f);
				...
				{
					if (ShouldStopDialing && ActiveChevrons > 0)
						return; // check if we should stop dialing or not
		...
		public override async void BeginDialSlow(string address, float initialDelay = 0)
		{
			...
			try
			{
				...
				foreach (var sym in address)
				{
					...
					var movieOffset = ChevronAngles[
						Chevrons.IndexOf(GetChevronBasedOnAddressLength(chevNum, address.Length))
					];
				...
				else
				{
					StopDialing();
					if (address.Length == 9)
						PlaySound(this, GetSound("dial_fail_9chev"), 0.5f);
				}
			}
			catch (Exception)
			{
		...
		public override void DoDHDChevronLock(char sym) // only the top chevron locks, always
		{
			...
			var valid = (
				gate != this
				&& gate.IsValid()
				&& gate.IsStargateReadyForInboundDHD()
				&& ChevronLightup
			);
		...
		public override async Task<bool> DoManualChevronEncode(char sym)
		{
			...
			// try to encode each symbol
			var movieOffset = -ChevronAngles[
				Chevrons.IndexOf(GetChevronBasedOnAddressLength(chevNum, chevNum))
			];
		...
		public override async Task<bool> DoManualChevronLock(char sym)
		{
			...
			var valid =
				target.IsValid() && target != this && target.IsStargateReadyForInboundInstantSlow();
			if (MovieDialingType)
			{
				ChevronAnimLockAll(chevNum, 0, ChevronLightup);
			}
