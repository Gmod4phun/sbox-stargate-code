namespace Sandbox.Components.Stargate.Rings
{
	...
	{
		public IEnumerable<RingPanelButton> Buttons =>
			Components.GetAll<RingPanelButton>(FindMode.InChildren);
		public SkinnedModelRenderer Renderer =>
			Components.Get<SkinnedModelRenderer>(FindMode.InSelf);
		...
		public void TriggerAction(string action) // this gets called from the Panel Button after pressing it
		{
			...
			if (TimeSinceButtonPressed < ButtonPressDelay)
				return;
			...
			if ((ValidButtonActions.Contains(action) && action.Length == 1) || action is "DIAL")
			{
				if (action is "DIAL") // we pressed dial button
				{
					...
					{
						Rings.DialRings(
							ComposedAddress.Length == 0
								? Rings.FindClosest()
								: FindRingtransporterByAddress(ComposedAddress),
							0.25f
						);
					}
		...
		{
			if (TimeSinceButtonPressed > 5 && ComposedAddress != "")
				ResetAddress();
		}
