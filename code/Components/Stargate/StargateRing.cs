public class StargateRing : PropertyChangeComponent, Component.ExecuteInEditor
{
	...
	private string _ringSymbols { get; set; } = "#0JKNTR3MBZX*H69IGPL@QFS1E4AU85OCW72YVD";
	...
	public virtual float GetSymbolPosition( char sym ) // gets the symbols position on the ring
	{
		...
		return _ringSymbols.Contains( sym ) ? _ringSymbols.IndexOf( sym ) : -1;
	}

	public virtual float GetSymbolAngle( char sym ) // gets the symbols angle on the ring
	{
		sym = sym.ToString().ToUpper()[0];
		return GetSymbolPosition( sym ) * (360f / _ringSymbols.Length);
	}

	public void RingSymbolThink() // keeps track of the current symbol under the top chevron
	{
		var symRange = 360f / _ringSymbols.Length;
		var symCoverage = (_currentRingAngle + symRange / 2f).UnsignedMod( symRange );
		var symIndex = ((int)Math.Round( (_currentRingAngle + _curRingSymbolOffset) / (symRange) )).UnsignedMod( _ringSymbols.Length );

		var threshold = 0.05f;
		var symCoverageThresholdMin = symRange * threshold;
		var symCoverageThresholdMax = symRange * (1 - threshold);

		CurRingSymbol = (symCoverage < symCoverageThresholdMax && symCoverage > symCoverageThresholdMin) ? _ringSymbols[symIndex].ToString() : "";
	}
	...
	public virtual float GetDesiredRingAngleForSymbol( char sym, float angOffset = 0 )
	{
		...
		if ( symPos == -1 ) return _currentRingAngle;
		...
		var D_CW = -symAng + _currentRingAngle + angOffset; // offset, if we want it to be relative to another chevron (for movie stargate dialing)
		var D_CCW = 360 - D_CW;
		...
		var finalAng = _currentRingAngle + (angToRotate * _ringDirection);
	...
	public async Task<bool> RotateToSymbol(char sym, float angleOffset = 0) {
		...
		var toTravelTotal = Math.Abs(desiredAngle - _currentRingAngle); // total angle needed to travel
		...
		while (true) {
			...
			if (_ringState == RingState.STOPPING) {
				...
				if (multiplier == 0) {
					...
					_currentRingAngle = _currentRingAngle.UnsignedMod(360f);
			...
			_currentRingAngle += toTravelStep * _ringDirection;
			_currentRingAngle = _currentRingAngle.UnsignedMod(360f);
	...
	{
		var deg = 360f / _ringSymbols.Length;
		var ang = Transform.Local.Rotation.Angles();
		foreach ( char sym in _ringSymbols )
		{
			var rotAng = ang.WithRoll( -ang.roll + (_ringSymbols.IndexOf(sym) * deg) );
