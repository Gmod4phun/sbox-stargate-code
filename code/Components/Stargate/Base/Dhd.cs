namespace Sandbox.Components.Stargate
{
	public class Dhd : Component, Component.ExecuteInEditor
	{
		...
		protected virtual Dictionary<string, Vector3> ButtonPositions =>
			new()
			{
				// Inner Ring
			...
			using (Gizmo.Scope("DhdSymbols", global::Transform.Zero))
			{
				foreach (var entry in ButtonPositions)
				{
					...
					var t1 = global::Transform
						.Zero.WithPosition(finalPos)
						.WithRotation(finalRot)
						.WithScale(0.022f);
					var t2 = global::Transform
						.Zero.WithPosition(finalPos + Vector3.Up * 0.01f)
						.WithRotation(finalRot)
						.WithScale(0.02f);
		...
		*/
		...
		{
			foreach (DhdButton b in Buttons)
		...
		public void EnableButtonsForDialingAddress()
		{
			...
			DisableAllButtons();
			foreach (char sym in Gate.DialingAddress)
				SetButtonState(sym.ToString(), true);

			if (Gate.Open || Gate.Opening || Gate.Closing)
			{
				var dial = GetButtonByAction("DIAL");
				if (dial.IsValid())
					SetButtonState(dial, true);
			}
		}
		...
		public async void TriggerAction(string action, GameObject user, float delay = 0) // this gets called from the Button Trigger after pressing it
		{
			...
			if (!Gate.IsValid())
				return; // if we have no gate to control, cant do much
			...
			if (Gate.Busy || Gate.Inbound)
				return; // if gate is busy, we cant do anything

			if (Gate.Dialing && Gate.CurDialType is not Stargate.DialType.DHD)
				return; // if we are dialing, but not by DHD, cant do anything

			if (action is not "DIAL") // if we pressed a regular symbol
			{
				if (PressedActions.Contains("DIAL"))
					return; // do nothing if we already have dial pressed
				if (!PressedActions.Contains(action) && PressedActions.Count is 9)
					return; // do nothing if we already have max symbols pressed
				if (!PressedActions.Contains(action) && action is "#")
				{
					if (PressedActions.Count < 6)
						return;
				}
				if (Gate.Opening || Gate.Open || Gate.Closing)
					return;
			}
			...
			if (action is "DIAL") // we presed dial button
			{
				...
				else // try dial
				{
					...
					if (
						target.IsValid()
						&& target != Gate
						&& target.IsStargateReadyForInboundDHD()
						&& Gate.CanStargateOpen()
						&& !Gate.IsLockedInvalid
					)
					{
						Stargate.PlaySound(
							MultiWorldSystem.GetWorldIndexOfObject(GameObject),
							Transform.Position + Transform.Rotation.Up * 16,
							Data.DialPressSound
						);
			...
			else // we pressed a symbol
			{
				...
				if (symbol != '#' && PressedActions.Contains("#"))
					return; // if # is pressed, and we try to depress other symbol, do nothing
