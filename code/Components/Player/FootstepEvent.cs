public sealed class FootstepEvent : Component
{
	...
	private void OnEvent( SceneModel.FootstepEvent e )
	{
		...
		var multiWorldSound = MultiWorldSound.Play( sound, tr.HitPosition + tr.Normal * 5, Player.CurrentWorldIndex );
		multiWorldSound.Volume = 0.5f;
	}

	public static void PlayJumpLandSound( PlayerController player, Vector3 position, bool landSound = false )
	{
		if ( !player.PlayerAlive )
			return;
		...
		var tr = player.Scene.Trace
			.Ray( position + Vector3.Up * 20, position + Vector3.Up * -20 )
			.WithTag( currentWorldTag )
			.Run();
