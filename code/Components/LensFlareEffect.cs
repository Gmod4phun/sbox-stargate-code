public class LensFlareEffect : PostProcess
{
    ...
    */
    ...
    public void DrawLensFlare( SceneCamera sceneCamera )
    {
        ...
        var tr = Scene.Trace.Ray( eyepos, eyepos + sundirection * 128000 ).WithTag( MultiWorldSystem.GetWorldTag( MultiWorldSystem.GetWorldIndexOfObject( GameObject ) ) ).Run();
        if ( tr.Hit )
            sunobstruction = 0;
        ...
        if ( DrawRing )
        {
            ...
            CenteredSprite( mulW( sunpos.x, 0.5f ), mulH( sunpos.y, 0.5f ), rSz * 8, Color.White.WithAlpha( 80 * aMulPow3 ) );
        }
        ...
        if ( DrawBar )
        {
            ...
            CenteredSprite( mulW( sunpos.x, 1 ), mulH( sunpos.y, 1 ), rSz * 5, Color.White.WithAlpha( 20f * aMul ) );
        }
